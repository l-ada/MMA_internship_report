...
\begin{figure}[h!]
 ...
 \caption{Comparison of training loss for Jacobian and equilibrium propagation approaches. Jacobian approach decreases loss at first, but starts diverging later. Equilibrium propagation seems to be more stable.}
...
\subsubsection{Summary of Jacobian appraoch}
 ...
 I have not been able to make Jacobian approach perform for the purpose of fine-tuning the model. Despite loss function decreasing during course of the training, when started from a different starting point the loss did not decrease. This suggests, that Jacobian approach did not in fact move the fixed point $\pth$ closer to real ground state $p_{gs}$ or did it while making energy landscape much more difficult to navigate. I include more details  in section \ref{jacmat}.


\section{ Equilibrium propagation}

\subsubsection{Base version}
During my internship I also implemented and tested optimization using Equilibrium Propagation (or EQ,EqProp, see \cite{eqprop}, \cite{zucchet2022beyond}), which is an alternative method of computing the gradient with respect to model parameters.
Let's denote
\begin{equation}
...
\end{equation}
\begin{remark}
 For $\beta>0$ the function $p \mapsto \beta \mathcal{L}(p)$ acts as a regularizer.
 \end{remark}
 \begin{remark}
...
\end{equation}
Since $T$ attains minimum at $\pthb$ we have
\begin{equation}
...
\begin{figure}[H]
 ...
 \caption{Training loss for different valus of $\beta$ parameter. The curves correspond to $\beta=0.001,0.01,0.1,1$ starting from the top.}
 % loss_comparison.png: 3002x2096 px, 300dpi, 25.42x17.75 cm, bb=0 0 720 50
 \label{fig:betas}
\end{figure}
